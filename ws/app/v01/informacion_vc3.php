			...
			if ($logueado) {
				...
				switch ($_GET['op']) {
					...
					case "dir" :
						...
						//$query = "SELECT idPersonal Id, CONCAT(IFNULL(sApPaterno,''),' ',IFNULL(sApMaterno,''),', ',sNombre) Nombre, CASE cTipoPers WHEN 'A' THEN 'Administrativo' WHEN 'D' THEN 'Director' WHEN 'P' THEN 'Profesor' WHEN 'S' THEN 'Subdirector' END Tipo, CASE cTipoPers WHEN 'A' THEN 3 WHEN 'D' THEN 1 WHEN 'P' THEN 4 WHEN 'S' THEN 2 END ordena FROM PCBPERSONAL WHERE cIndDirTelCel='S' OR cIndDirTelPriv='S' ORDER BY ordena Asc, Nombre Asc";

						$query = "SELECT idPersonal Id, CONCAT(IFNULL(sApPaterno,''),' ',IFNULL(sApMaterno,''),', ',sNombre) Nombre, cTipoPers Tipo,
									CASE cTipoPers WHEN 'D' THEN 1 WHEN 'S' THEN 2 WHEN 'A' THEN 3 WHEN 'P' THEN 4 END Orden
									FROM PCBPERSONAL WHERE cIndDirTelCel='S' OR cIndDirTelPriv='S' ORDER BY Orden Asc, Nombre Asc";
					...
					case "det" :
						...
						$query = "SELECT CONCAT(IFNULL(sApPaterno,''),' ',IFNULL(sApMaterno,''),', ',sNombre) Nombre, cTipoPers Tipo,
