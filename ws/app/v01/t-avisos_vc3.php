				...
				$row_array = array();
					...
					switch ($op){
						case "gen" :
							...
							$query = "SELECT DISTINCT M.idMensAvi Id, M.sDCMensajeAviso Titulo, DATE_FORMAT(M.dFAlta,'%d-%m-%Y') Fecha, 'M' Tipo
											FROM PCSUSUARIO U 
											...
											UNION
											SELECT AL.idAlerta Id, CONCAT('[Alerta] - ',AL.aTituloAlerta) Titulo, DATE_FORMAT(AL.dFAlta,'%d-%m-%Y') Fecha, 'A' Tipo
											FROM PCBALERTA AL
						...
						case "det" :
							...
							if ($tip=='M'){
								$query = "SELECT M.idMensAvi Id, M.sDCMensajeAviso Titulo, DATE_FORMAT(dFAlta,'%d/%m/%Y') Fecha, M.sDMensajeAviso Contenido, CASE WHEN R.cTipoRespuesta IS NULL THEN '' ELSE R.cTipoRespuesta END Resp, 'M' Tipo 
											FROM PCBMENSAVI M
											LEFT JOIN PARRESPAPP R ON R.FKidMensAvi=M.idMensAvi
											WHERE idMensAvi='".$id."'";
							}else{
								$query = "SELECT AL.idAlerta Id, CONCAT('[Alerta] - ',AL.aTituloAlerta) Titulo, DATE_FORMAT(AL.dFAlta,'%d-%m-%Y') Fecha, AL.sDAlerta Contenido, CASE WHEN R.cTipoRespuesta IS NULL THEN '' ELSE R.cTipoRespuesta END Resp, 'A' Tipo
												FROM PCBALERTA AL
						...
						if ($tip='M'){
							...
							respApp($cnnDBCUSTOM,$usr, $id, $r, 't-alerta');
						}
